...
const AddProduct = () => {
  ...
  const [previewUrls, setPreviewUrls] = useState<MediaPreview[]>([]);
                  ...
                  <div className="flex flex-wrap gap-2">
                    {colorOptions.map(color => (
                      <button
                        ...
                        className={`px-3 py-1 rounded-full text-sm font-medium transition-colors ${formData.colors.includes(color)
                            ? 'bg-blue-500 text-white'
                            : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                          }`}
                      >
                    ...
                    <div className="mt-6">
                      ...
                      <div className="flex overflow-x-auto pb-2 gap-2">
                        {previewUrls.map((item, index) => (
                          <div
                            key={index}
                            className={`relative flex-shrink-0 w-16 h-16 rounded-md cursor-pointer border-2 transition-all ${index === activePreviewIndex
                                ? 'border-blue-500'
