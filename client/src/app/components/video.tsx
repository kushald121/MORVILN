  ...
  return () => {
    <div className="w-full bg-black text-white font-sans">
      ...
      <div className="w-full bg-black overflow-hidden">
        <div className="grid grid-cols-3 gap-0 w-full h-[360px] md:h-[520px]">
          {/* Left: play from middle */}
          <div className="w-full h-full overflow-hidden">
            <video
              ref={(el) => (videoLeftRef.current = el)}
              className="w-full h-full object-cover"
              src="/video1.mp4"
              muted
              playsInline
              // allow normal looping
            />
          </div>
          ...
          <div className="w-full h-full overflow-hidden">
            <video
              ref={(el) => (videoCenterRef.current = el)}
              className="w-full h-full object-cover"
              src="/video1.mp4"
              muted
              playsInline
          ...
          <div className="w-full h-full overflow-hidden">
            <video
              ref={(el) => (videoRightRef.current = el)}
              className="w-full h-full object-cover"
              src="/video1.mp4"
              muted
              playsInline
