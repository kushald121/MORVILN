...
if (!process.env.GOOGLE_CLIENT_ID || !process.env.GOOGLE_CLIENT_SECRET) {
  ...
  console.warn(
    "   Google login will not work without: GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET"
  );
}
if (!process.env.FACEBOOK_APP_ID || !process.env.FACEBOOK_APP_SECRET) {
  console.warn("⚠️  WARNING: Missing Facebook OAuth credentials in .env file");
  console.warn(
    "   Facebook login will not work without: FACEBOOK_APP_ID, FACEBOOK_APP_SECRET"
  );
}
exports.googleConfig = {
...
exports.facebookStrategy = new passport_facebook_1.Strategy(
  exports.facebookConfig,
  async (accessToken, refreshToken, profile, done) => {
    try {
      const user = {
