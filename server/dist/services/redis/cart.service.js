...
class CartService {
    constructor() {
        this.CART_EXPIRY = 5 * 24 * 60 * 60; // 5 days
    }
    // Add item to guest cart
    async addItem(sessionId, variantId, quantity = 1) {
        try {
            const cartKey = `cart:${sessionId}`;
            // Check if variant exists in cart
            const existingQuantity = await redis_1.redis.hget(cartKey, variantId);
            if (existingQuantity) {
                const currentQuantity = parseInt(existingQuantity);
                await redis_1.redis.hset(cartKey, { [variantId]: currentQuantity + quantity });
            }
            else {
                await redis_1.redis.hset(cartKey, { [variantId]: quantity });
            }
            // Set expiration
            await redis_1.redis.expire(cartKey, this.CART_EXPIRY);
    ...
    async getCart(sessionId) {
        try {
            const cartKey = `cart:${sessionId}`;
            const cartItems = await redis_1.redis.hgetall(cartKey);
            if (!cartItems || Object.keys(cartItems).length === 0) {
                return { success: true, message: 'Cart is empty', items: [] };
            }
            // Convert to array format with proper typing
            const items = Object.entries(cartItems).map(([variantId, quantity]) => ({
                variantId,
    ...
    async updateQuantity(sessionId, variantId, quantity) {
        try {
            ...
            if (quantity <= 0) {
                ...
                await redis_1.redis.hdel(cartKey, variantId);
            }
            else {
            ...
            // Verify the update
            const updatedCart = await redis_1.redis.hgetall(cartKey);
    ...
    async removeItem(sessionId, variantId) {
        try {
            const cartKey = `cart:${sessionId}`;
            await redis_1.redis.hdel(cartKey, variantId);
